...
<div class="max-w-3xl mx-auto">
    <h1 class="text-3xl font-semibold text-gray-800 mb-6">Edit Client</h1>

    <form method="POST" action="{{ route('clients.update', $client) }}" class="bg-white shadow rounded-lg p-6 space-y-5">
        @csrf
        @method('PUT')

        <!-- Name -->
        <div>
            <label class="block mb-1 text-gray-700 font-medium">Name <span class="text-red-500">*</span></label>
            <input 
                type="text" 
                name="name" 
        ...
        <div>
            ...
            <input 
                type="text" 
                name="activite" 
                value="{{ old('activite', $client->activite) }}" 
                class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring focus:ring-blue-200"
            >
